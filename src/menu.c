...
static void menu_paint_cell(int index)
{
	...
	int ww = g_rows == g_entries ? LCD_WIDTH : COL_W;
	int xx = ww * col + col;
...
//    uint16_t bgCol = COL_BG_NORM;
    ...
    g_rowh = (LCD_HEIGHT - CRUMB_H) / g_rows;

    //printf("Rows: %d rowh: %d entries: %d\r\n", g_rows, g_rowh, g_entries);

    // draw the grid
    lcd_fill(COL_W, CRUMB_H, 1, LCD_HEIGHT - CRUMB_H, 0xFFFF);
    for (ii = 0; ii < g_rows - 1; ii++)
		lcd_fill(0,  CRUMB_H + (ii +1) * (g_rowh) - 1, LCD_WIDTH, 1,    0xFFFF);
...
static void menu_update(void)
{
    ...
    // clear menu bg
    lcd_fill(0, CRUMB_H, LCD_WIDTH, LCD_HEIGHT - CRUMB_H, bgCol);
    ...
    lcd_fill(0, 0, LCD_WIDTH, CRUMB_H, 0x0);
    lcd_text(0, 0, crumbs);
	lcd_fill(0, CRUMB_H - 2, LCD_WIDTH, 2, 0xFFFF);
